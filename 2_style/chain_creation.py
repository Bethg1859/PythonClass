...
"""
This tool allows artists to easily create interlocked chain links in Maya through a user-friendly GUI.
Users can select from predefined chain shapes or add new custom shapes. The tool provides options
to adjust the scale, link count, and offset of the chain elements. 

Main Features:
- **Shape Management**: Load default shapes from an asset folder or add new shapes as needed.
- **Chain Customization**: Modify the chain's link count, link scale (X, Y, Z), and offset percentage along the Z-axis.
- **Automatic Instancing**: Each link is created as an instance of the base shape, keeping the scene lightweight and efficient.

Usage:
1. Launch the tool by running `create_chain_tool_gui()`.
2. Select a chain shape, adjust parameters, and click "Create Chain" to generate the chain.
3. Use "Add New Base Shape" to add a custom shape to the asset folder and populate it in the dropdown menu.

This tool is ideal for artists needing flexible, lightweight chain creation for models or animations.
"""
